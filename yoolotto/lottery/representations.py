...
class TicketRepresentationShortList(TicketRepresentation):
	...
	app_version = self.app_version
		...
		user = YooLottoUser.objects.get(email=coins_info.email)
                ...
                iphone_version  = ["8.5","8.4","8.3"]
                android_version = ["8.6","8.5","8.4"]
                if (app_version in iphone_version and device_type == "IPHONE") or (app_version in android_version and device_type == "ANDROID"):
                    if self.ticket.ticket_submissions.all()[0].pending != 0:
                        if int(self.ticket.ticket_submissions.all()[0].gameType) == 0 or int(self.ticket.ticket_submissions.all()[0].gameType) == 13:
                            if coins_info.coins >= count *1:
                                ...
                                coins_info.save()
                                user_coins_spend_info = UserCoinsSpendHistory(user = user,coins= count*1,source="mm_ticket")
                                user_coins_spend_info.save()
                                ticket_available = LotteryTicketAvailable.objects.filter(ticket_id = self.ticket.id)
                                for tic in ticket_available:
                                    tic.pending = 0
                                    tic.save()
                                user_coins_spend_info,created= FantasyLotterySpentHistory.objects.get_or_create(user_id = user.id,coins= count*1,source="megamillion",ticket_id = self.ticket.id)
                            ...
                            if coins_info.coins >= count *2:
                                ...
                                coins_info.dollar_amount = coins_info.dollar_amount - amount
                                coins_info.save()
                                user_coins_spend_info,created = FantasyLotterySpentHistory.objects.get_or_create(user_id = user.id,coins= count*2,source="powerball",ticket_id = self.ticket.id)
                                user_coins_spend_info = UserCoinsSpendHistory(user = user,coins= count*2,source="pb_ticket")
                                user_coins_spend_info.save()
                                ticket_available = LotteryTicketAvailable.objects.filter(ticket_id = self.ticket.id)
                                for tic in ticket_available:
                                    tic.pending = 0
